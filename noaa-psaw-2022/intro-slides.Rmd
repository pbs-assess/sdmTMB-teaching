...
*  New features to suggest? Add to issue tracker:  
  [https://github.com/pbs-assess/sdmTMB/issues](https://github.com/pbs-assess/sdmTMB/issues)
...
* sdmTMB uses an approach from INLA
  ...
  * INLA books: [https://www.r-inla.org/learnmore/books](https://www.r-inla.org/learnmore/books)
...
* INLA is software that performs data wrangling for SPDE estimation
  * INLA also performs fast approximate Bayesian estimation (Laplace approximation)
...
ggplot() + inlabru::gg(mesh$mesh) +
  geom_point(data = pcod, aes(x = X, y = Y), alpha = 0.3, pch = 20) +
  coord_equal()
```
...
* A need for reproducible code and methods for assessments [(Anderson et al. 2019)](https://publications.gc.ca/collections/collection_2020/mpo-dfo/fs70-5/Fs70-5-2019-041-eng.pdf)
...
```{r make-breakpt, fig.asp=0.45, fig.width=9}
...
```{r make-logistic, fig.asp=0.45, fig.width=9}
df <- data.frame(x = seq(1, 100))
slope <- 0.1
...
r2 <- fishMod::rTweedie(20000, mu = 3, phi = 1, p = 1.3)
r3 <- fishMod::rTweedie(20000, mu = 3, phi = 10, p = 1.3)
...
* Like `glm()` and `glmmTMB()`, `sdmTMB()` allows the use of offsets and weights:
...
Note that `offset` is currently a reserved word!
...
* Why would a field be IID vs RW/AR1?
  * Do we expect hotspots to persist or adapt slowly over time?
