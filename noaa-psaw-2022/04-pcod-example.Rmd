...
nd <- data.frame(depth = seq(30, 500, length.out = 100))
p <- predict(
  fit, 
  newdata = nd, 
  se_fit = TRUE, #<<
...
.xsmall[
```{r pcod-st-index-sims2, echo=TRUE, fig.width=6, fig.asp=0.5, out.width="600px"}
qcs_grid2 <- qcs_grid %>% filter(depth<=400) #<<
psims <- predict(fit, newdata = qcs_grid2, nsim = 1000) 
index_sims <- get_index_sims(psims, area = rep(4, nrow(psims))) 
ggplot(index_sims, aes(year, est)) + geom_line() +
  geom_ribbon(aes(ymin = lwr, ymax = upr), alpha = 0.4) +
  xlab('Year') + ylab('Biomass estimate (kg)')
