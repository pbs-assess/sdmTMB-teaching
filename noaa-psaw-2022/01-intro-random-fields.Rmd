...
* Have questions? Use the Google Doc, ask during our break, or at the end. Thanks!

---

<!-- high level intro trhing... 
- wiggly latent field
- glm
- plot residuals...
- need wiggly surface...
- random fields...
- 1D is gaussian process
-->
...
x <- seq(1, 50, length.out = 300)
d <- as.matrix(dist(x, diag = TRUE))
set.seed(123)
df <- data.frame(
  x = x,
  true = c(spam::rmvnorm(1, mu = 0, Sigma = 2 * exp(-30 * d)))
)
df$y <- rnorm(nrow(df), df$true, 0.1)
...
TODO: make smooth; just wiggly lines?
...
# TODO: overlay knots? vertical lines
```
...
# Visualize difference sigma + covariance
