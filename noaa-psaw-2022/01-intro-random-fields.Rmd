...
df = dplyr::group_by(df, row) %>% 
  dplyr::mutate(cov = matern.covariance(h=x,kappa=5,nu=nu,sigma=1))
df$nu = as.factor(df$nu)

ggplot(df, aes(x, cov, col = nu, group=nu)) +
  geom_line(size=1.3,alpha=0.8) +
  theme_bw() +
  xlab("Distance") + ylab("Covariance") + ggtitle("Matérn covariance") +
...
* `make_mesh()` function allows 2 types of meshes

* K-means algorithm used to cluster points (user specifies `type="kmeans` and `n_knots` chosen)

* Minimum cutoff (min. allowed distance between points) (`type=cutoff` and `cutoff` chosen)

---

# Original example, cutoff = 10km
