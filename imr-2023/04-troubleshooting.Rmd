...
* So, small values are consistent with convergence (no hard rule, perhaps at least < 0.001 but depends on scale of likelihood)
...
It all depends on data (distance between survey points, etc) but check that that estimated `Matern range > mesh size`
...
* Make sure the estimated spatial range > mesh size 
