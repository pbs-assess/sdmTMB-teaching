...
- Becomes familiar with spatiotemporal random fields and how they can combine
  with spatial random fields.
...
The spatial range is the distance at which two data points are effectively independent. This is the distance at which correlation declines to approximately 0.13.

We can visualize the implied Matern correlation function for various range values:
...
Try simulating some random fields.

First, this chunk creates a set of X and Y spatial coordinates and constructs a mesh using a minimum triangle edge length cutoff distance of 0.05.
...
-   The formula here contains an intercept (but could include covariates)

-   The family is Gaussian to specify that we simulate a normally distributed response.

-   The range (or distance at which two data points are effectively independent; correlation declines to \~0.13) is 0.6.
...
As a reminder, the range is scaled based on the units of the locations---here the X and Y variables range between 0 and 1 and are unitless, but for other data the range may be different or in units of m or km, for example.

**Exercise:**

1.  **Try running the previous chunk over and over. Try changing the `range` and `sigma_O` values. What do you observe?**
2.  **Try increasing the observation error standard deviation `phi`. Re-run it many times. Try making it equal to `sigma_O` and twice its size. What happens when observation error is much larger than `sigma_O`**?
...
2.  **What do you notice about `omega_s` (spatial random field) across the time slices? Conversely, what do you notice about `epsilon_st` across time?**
...
sim_dat3 <- sdmTMB_simulate(
  ...
  sigma_O = 0,
  sigma_E = 0.1,
  spatiotemporal = "ar1", #< field type (default: IID or independent across years)
  rho = 0.99, #< AR(1) field correlation
  phi = 0.001,
  B = 0 # intercept
)
...
1.  **In the previous chunk try setting the AR(1) random field correlation to values ranging from 0 to a value near 1 (e.g., 0.99). What do you observe?**
