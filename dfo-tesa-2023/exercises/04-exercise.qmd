...
- log(weight/area_swept) = b0 + b1 * x
- log(weight) - log(area_swept) = b0 + b1 * x
- log(weight) = b0 + b1 * x + log(area_swept) 
- log(weight) = b0 + b1 * x + offset

Note the offset if the **log** of the effort variable.

Here, we could fit our model to catch weight with an area swept offset or to density. In other cases (e.g., count data), an offset lets us use a count distribution with an effort value.
...
```{r}
d$resid_sp <- residuals(fit_spatial) # exercise
...
Use `get_index()` to calculate an area-weighted biomass index. Feel free to set `bias_correct = FALSE` to be a bit faster.
