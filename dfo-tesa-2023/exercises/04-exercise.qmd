...
Add UTM columns to the dataset. Use `utm_crs = 32609` to match the UTM zone 9 used in the grid.

```{r}
d <- add_utm_columns(d, units = "km", utm_crs = 32609) # excercise
```
...
Did that fit reasonably well such that we can try adding the complexity of spatiotemporal fields? In addition to the fits, do the spatial residuals in any year show evidence of persistent spatial autocorrelation?  
...
2.  Is the spatial or spatiotemporal variance larger? Does that make sense? What does this mean biologically?  
