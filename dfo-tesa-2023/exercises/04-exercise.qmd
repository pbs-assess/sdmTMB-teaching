...
```{r}
fit$gradients[fit$gradients > 0.001]
```

```{r}
fit <- run_extra_optimization(fit, newton_loops = 1) # exercise
```

You could also have used in the first place, this does the same thing but is a bit faster to do it all at once. See `?sdmTMBcontrol` and the `newton_loops` argument.
...
```{r}
ind <- get_index(pred, area = 4, bias_correct = TRUE) # exercise
```
...
```{r}
mesh10 <- make_mesh(pcod, xy_cols = c("X", "Y"), cutoff = 10)
plot(mesh10)
```
...
```{r}
fit_ye1 <- update(fit_ye, spatiotemporal = "off")
sanity(fit_ye1)
fit_ye1
```
...
What is wrong with the above? How can you fix it?
