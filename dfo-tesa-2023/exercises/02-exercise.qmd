...
We can add UTM columns using the `add_utm_columns()`:
...
We will re-fit out model with REML so we can use AIC to compare models with different random effect structures. We will then fit a model with anisotropy enabled. We will use the handy method `update()` here, but we could also re-write the whole `sdmTMB()` function call each time.
