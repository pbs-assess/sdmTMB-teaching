...
3.  What other delta families are there in sdmTMB? See `?sdmTMB::Families`. Which other ones could be appropriate here?
...
And it appears that including the spatiotemporal field in both components (`fit_dg`) is more important than the anisotropy (`fit_dg_aniso`). These models are close enough, however, that we would probably chose a model for reasons other than AIC.
