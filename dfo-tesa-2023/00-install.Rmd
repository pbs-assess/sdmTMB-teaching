...
**As of January 11, only the macOS binary version is ready. If on Windows, wait another day or two or go the slower route of installing from source.**

```{r, eval=FALSE}
install.packages("sdmTMB", dependencies = TRUE)
# if you don't want to wait a day or two for the Windows binary version to be
# ready, you can try:
# install.packages("sdmTMB", dependencies = TRUE, type = "source")
```
