...
# Formulas as lists for multivariate responses

* e.g. model size-at-age and weight-at-age-simultaneously 
```{r argdemo2, eval=FALSE, echo=TRUE}
fit(..., 
formula = list(length ~ depth, 
    weight ~ depth + s(temp)),
family = list('lognormal', 'lognormal'),...)  
```

* currently little benefit, as sdmTMB shares no information between these models (e.g. no shared spatial / spatiotemporal fields)
...
* Implemented with `gamma_mix` and `lognormal_mix`  

* Can be combined with delta model as `delta_gamma_mix` and `delta_lognormal_mix`
