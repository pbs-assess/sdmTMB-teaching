...
These are the predictive densities for each fold. Larger (more positive) values are better. This gives us an idea of the variance of these values across folds.
...
These are the sum of the predictive densities for all left out data combined.
...
1.  Which model does the loglik suggest has better out-of-sample predictive performance?
...
These don't look great, *but*, this is largely a function of error from the Laplace approximation on the random effects (see [Thygesen et al. 2017](https://doi.org/10.1007/s10651-017-0372-4)). MCMC-based are a better approach, but are slower. There's a whole vignette on residual checking with sdmTMB here: <https://pbs-assess.github.io/sdmTMB/articles/residual-checking.html> It's an active area of research.

Further, we can simulate new observations from the model and check properties of the simulation compared to the observed data:
