...
touse <- c("here","tidyverse","sdmTMB","sf","rnaturalearth","viridis","ggOceanMaps","alphahull","ggspatial")
lapply(touse, require, character.only=TRUE, quietly=TRUE)
plotdir <- here('amaremed-2024','exercises','03-imr-coastal-survey-ex','plots','south')
resdir <- here('amaremed-2024','exercises','03-imr-coastal-survey-ex','results','south')
...
dfc <- readRDS(here('amaremed-2024','data','survey_catch_clean_stox.rds'))
...
mesh <- make_mesh(dfc_south, xy_cols = c("X","Y"), cutoff = 3) # CONSIDER INCREASING CUTOFF FOR SPEED
...
index.stox <- read.csv(here('amaremed-2024','data','coastalcod_totalB2plus_tonn_swept_south.csv'))
head(index.stox)
